...
class GRL(Function):
    ...
    def forward(ctx, input):
        ctx.alpha = torch.tensor(0.1, dtype=torch.float32, requires_grad=True)
        return input.view_as(input)

    @staticmethod
    def backward(ctx, grad_outputs):
        output = grad_outputs.clone().neg() * ctx.alpha
        return output
...
class FUA(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(256, 512, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.conv_en2 = nn.Conv2d(128, 1, kernel_size=3, stride=1, padding='same', bias=False)
